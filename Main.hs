...
parseEventData cls = do
    l <- getLine
    if l == "</event>"
        then putStrLn "Done reading in jets." >> writeJets cls {-(aktJets 0.4 cls)-} >> readDataFromRoot
...
writeJets jets = mapM_ (putStrLn . show . toPtEtaPhiE . getData) jets >> (putStrLn (show (mindij 0.4 (-1) jets)))
